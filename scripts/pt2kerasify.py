#!/usr/bin/env python3
"""CLI: convert a trained PyTorch encoder model to Keras format and export via Kerasify."""
...
def convert_pt_to_keras(encoder: nn.Sequential) -> "keras.models.Sequential":
    ...
    """
    if keras is None:
        raise ImportError("Keras is not installed. Please install keras to convert models.")

    input_shape = [encoder[0].in_features]
    n_units_lst = [layer.out_features for layer in encoder if not isinstance(layer, nn.ReLU)]
...
def main() -> None:
    ...
    parser.add_argument(
        "out_model", type=str, help="Path to write the exported Kerasify model file."
    )
